...
sys.path.insert(1, 'C:/Users/' + pc + '/OneDrive - UGent/master 2/thesis/code/XRFPM_b')

figuresaver = "/Users/Pedro/OneDrive - UGent/master 2/thesis/figures/"


import XRFPM_b as XRFPM
...
calibration_file = 'La Calibration.csv'
mineralphase = XRFPM.create_minerals(path_csv, calibration_file, 21, calculate_major=True)

trial_pixels = XRFPM.pixelisation(array=trial_data)
...
# plt.imshow(img)
# plt.savefig(figuresaver+'Apatite/overview_image', bbox_inches="tight")
# plt.show()
...
# pix = XRFPM.create_pixels(data, phases, path2, apatite_data, path_min=path_csv, csv=minerals_file, energy=100, err=20, ps_cor=ps_cor, pixel_size_x=2, pixel_size_y=2)
...
# #create pixels
# pix = XRFPM.create_pixels(data, phases, array=im, path_min=path_csv, csv=minerals_file, energy=energy, err=10, ps_cor=ps_cor, pixel_size_y=1, pixel_size_x = 1)
...
# #create pixels
# pix = XRFPM.create_pixels(data, phases, array=im, path_min=path_csv, csv=minerals_file, energy=energy, err=5, ps_cor=ps_cor, pixel_size_y = 1, pixel_size_x = 1, calculate_major=True)
# detected_phases = XRFPM.rejoin_map_ct(pix, 200)
# sino = XRFPM.sinogram_rotation_2d(i0, Adet, Dsd, element, line, energy, pix, im_size, theta)
# recon = XRFPM.reconstruction_sinogram_2d(sino, theta)
...
# #create pixels
# pix = XRFPM.create_pixels(data, phases, array=im, path_min=path_csv, csv=minerals_file, energy=energy, err=5, ps_cor=ps_cor, pixel_size_y = 1, pixel_size_x = 1)
# detected_phases = XRFPM.rejoin_map_ct(pix, 200)
# sino = XRFPM.sinogram_rotation_2d(i0, Adet, Dsd, element, line, energy, pix, im_size, theta)
# recon = XRFPM.reconstruction_sinogram_2d(sino, theta)
...
#create voxels
vox = XRFPM.create_voxels(data, phases, array=im, path_min=path_csv, csv=minerals_file, energy=energy, err=20, ps_cor=ps_cor, rows=1, cols=1, depth=1)
...
# #create voxels
# vox = XRFPM.create_voxels(data, phases, array=im, path_min=path_csv, csv=minerals_file, energy=energy, err=20, ps_cor=ps_cor, rows=2, cols=2, depth=2)
# # detected_phases = XRFPM.rejoin_vox_grid(vox, im_size) #doesnt work because image is too large
# sino = XRFPM.sinogram_rotation_3d(i0, Adet, Dsd, element, line, energy, vox, im_size, theta)
# XRFPM.display_ct_scans_3d(arrays=sino, opacity=0.1)
...
#     #create pixels
#     pix = XRFPM.create_pixels(data, phase, array=im, path_min=path_csv, csv=calibration_file, energy=energy, err=5, ps_cor=ps_cor, pixel_size_y = 1, pixel_size_x = 1, calculate_major=True)
#     detected_phases = XRFPM.rejoin_map_ct(pix, im_shape[1])
#     sino = XRFPM.sinogram_rotation_2d(i0, Adet, Dsd, element, line, energy, pix, im_size, theta)
#     recon = XRFPM.reconstruction_sinogram_2d(sino, theta)
