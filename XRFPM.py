...
"""
Pixelisation/Voxelisation functions
"""
...
def rejoin_map_ct(Pixels, size_cols, viridis=False):
    ...
    if viridis is True:
        ...
        cmap = plt.get_cmap('tab10', int_unique_categories)
...
def Geometryfactor_2d(indexmatrix, Adet, Dsd, ps_cor, angle):
    """
    Calculate the geometryfactor taking into account the tilt angle between pixel and detector center
...
def Geometryfactor_3d(indexmatrix, Adet, Dsd, ps_cor, angle, detector_side='Left'):
    """
    Calculate the geometryfactor taking into account the tilt angle between voxel volume and detector center,
...
def FP_slice(density, thickness, concs, mu_o, mu_f, detector_side='Left'):
    ...
    abs_x_slice = np.cumprod(np.exp(-mu_o[::-1] * thickness[::-1] * density[::-1]), axis=0)[::-1]
    
    
    abs_y_slice = np.cumprod(np.exp(-mu_f[slicestyle] * thickness[slicestyle] * density[slicestyle]), axis=1)[slicestyle]
...
def I_fluor_conv(I0, G, Qi, w_mu, fluormatrix, absorption_x, absorption_y): #for voxels
    """
    Calculate theorhetical intensity of specific xrf line for chosen element,
        uses fpm formula derived from tom schoonjans for conventional setup,
...
def I_fluor_confocal(I0, G, Qi, w_mu, fluormatrix, absorption_x, absorption_y): #for voxels
    """
    Calculate theorhetical intensity of specific xrf line for chosen element,
        uses fpm formula from tom schoonjans,
            from precalculated fpm parameters 
...
def sinogram_rotation_3d(I0, Adet, Dsd, element, line, energy, Voxels, image_size, theta, dualdet=False): #for voxels
    """
    Create a sinogram of XRF intensities calculated using FPM method for voxels, works per slice
...
def quantify_2d(I0, Adet, Dsd, element, line, energy, Pixels, image_size, theta, experimental_sinogram, error= 5, dualdet=False, algo='fbp'):
    ...
    reconstruction = reconstruction_sinogram_2d(sinogram, theta, algo=algo)
   
    #make reconstructed image based on experimnental data
    recon_real = reconstruction_sinogram_2d(experimental_sinogram, theta, algo=algo)
    ...
    slope, intercept, r = sc.stats.linregress(concentration.ravel(), reconstruction.ravel())[:3]
    
    assert r >= 0.8, r"Correlation is too low for (semi)-quantitative purposes ($R^2$ = {r}) "
        #correlation between concentration and reconstruction is high enough
            #start iterative scheme
                #unpad concentration
    mask = rows_index != -1
    concentration_unpad = concentration[mask]
    reconstruction_unpad = reconstruction[mask]
    
    assert reconstruction_unpad.shape == recon_real, "Failed unpadding: Experimental data and simulated data have different shapes."
    iterations = 0
    
    while np.mean((100* (recon_real - reconstruction_unpad) / recon_real)) > error or iterations > 100:
        
        for r, row in enumerate(recon_real):
            for n, iexp in enumerate(row):
                isim = reconstruction_unpad[r,n]
                wi0 = concentration_unpad[r,n]
                wi1 = wi0* (iexp / isim)
                
                Pixels[rows_index[r,n]].conc[element] = wi1
...
def density(densities, weightfractions, ppm=True):
    """
    Calculate density of mixture using weightfractions of pure compounds and their densities

    Parameters
    ----------
    densities : list(int)
        Array of densities of the pure compounds in the mixture
    weightfractions : list(int)
        Array of weightfractions of the pure compounds in the mixture
    ppm : TYPE, boolean
        True if weightfraction is given in ppm concentrations instead of mass percentage, DEFAULT is True
